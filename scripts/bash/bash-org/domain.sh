...
if [ $ok ] ; then
  ...
  do
     ...
     then
       ...
       expiration=$(echo "$VAL" | awk -F: '/Registry Expiry Date:/{print substr($0, length($1) + 3)}')
       nap=${expiration:8:2}
       honap=${expiration:5:2}
       ev=${expiration:0:4}
       echo "$ev-$honap-$nap"
       mongo localhost/mdac --eval "db.domains.insert({name: '$domain', tld: '$1', taken: 1, expiration: '$ev-$honap-$nap'});"
     fi
