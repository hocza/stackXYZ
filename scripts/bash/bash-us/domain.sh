...
if [ $ok ] ; then
  ...
  do
     ...
     then
       ...
       mongo localhost/mdac --eval "db.domains.insert({name: '$domain', tld: '$1', taken: 0, expiration: ''});"
     else
       echo -e "\e[31m$domain.$1 TAKEN :-("
       expiration=$(echo "$VAL" | awk -F: '/Expiration Date:/{print substr($0, length($1) + 3)}')
       echo $expiration
       str2=${expiration:25:6}
       str3=${expiration:45:4}
       echo $str3
       exp=`date --date="$str2 $str3" '+%Y-%m-%d'`
       echo $exp
       mongo localhost/mdac --eval "db.domains.insert({name: '$domain', tld: '$1', taken: 1, expiration: '$exp'});"
     fi
